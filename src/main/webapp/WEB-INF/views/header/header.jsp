...
<nav class="navbar navbar-default">
	<div class="container">
		...
		<div class="navbar-header">
			...
			<a href="<c:url value="/"  />"><img class="navbar-brand"
				src="<c:url value="/resources/images/logo.png" />"
				href="<c:url value="/"  />"></img></a>
		</div>
		...
		<div class="collapse navbar-collapse" id="navbar-collapse-2">
			...
			<ul class="nav navbar-nav navbar-right">
				...
				<li class="dropdown"><a href="#" class="dropdown-toggle"
					data-toggle="dropdown" role="button" aria-expanded="false">${partners}
				...
				<li class="dropdown"><a href="#" class="dropdown-toggle"
					...
					<ul class="dropdown-menu" role="menu">
						<li><security:authorize access="hasRole('ROLE_GUEST')">
								<a href="<c:url value="/signin" />"'> ${signin} <i
									class="fa fa-sign-in"></i>
								</a>
								<a href="<c:url value="/signup" />"'> ${signup} <i
									class="fa fa-user-plus"></i>
								</a>
							</security:authorize> <security:authorize
								access="hasAnyRole('ROLE_DESIGNER','ROLE_USER', 'ROLE_MODERATOR', 'ROLE_ADMIN')">
								<a href="<c:url value="${profileURL}" />"'> ${profile} <i
									class="fa fa-suitcase"></i>
								</a>
							</security:authorize></li>
						<li><security:authorize
								access="hasAnyRole('ROLE_DESIGNER','ROLE_USER', 'ROLE_MODERATOR', 'ROLE_ADMIN')">
								<a href="<c:url value="/logout" />"'> ${logout} <i
