...
<body>
	<form:form modelAttribute="postForm" method="POST" id="add-post-form"
		...
		<form:select id="category_idCategory" name="category_idCategory"
			path="category_idCategory">
			<c:forEach items="${listCategory}" var="category">
				<form:option value="${category.idCategory}">${category.title}</form:option>
				<c:set value="${category.subcategories}" var="subcategories"></c:set>
			</c:forEach>
		</form:select>
		<form:select id="subcategory_idSubcategory"
			name="subcategory_idSubcategory" path="subcategory_idSubcategory">
